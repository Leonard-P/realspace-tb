...
class NeumannSolver(ABC):
    ...
    def _step(
        ...
        """Perform a single order step to evolve the density matrix"""
        raise NotImplementedError("Every ODE Solver must have _step method!")
    ...
    def evolve(
        ...
        for step in trange(n_steps, disable=not progress):
            ...
            self._step(t, rho, H, dt, rho_0, tau)

        for observable in observables or []:
            observable.finalize()


class RK4NeumannSolver(NeumannSolver):
    ...
    def _step(
        ...
        """
        k1 = self._time_evolution_derivative(t, rho, H.at_time(t), rho_0, tau)

        k2 = self._time_evolution_derivative(
            t + 0.5 * dt, rho + 0.5 * dt * k1, H.at_time(t + 0.5 * dt), rho_0, tau
        )
        k3 = self._time_evolution_derivative(
            t + 0.5 * dt, rho + 0.5 * dt * k2, H.at_time(t + 0.5 * dt), rho_0, tau
        )
        k4 = self._time_evolution_derivative(
            t + dt, rho + dt * k3, H.at_time(t + dt), rho_0, tau
        )

        rho += (1.0 / 6.0) * dt * (k1 + 2 * k2 + 2 * k3 + k4)
